...
{
    if (rewardCount < 20)
    { // if the quantity is less than 20 rewards
...
void showRewards()
{
    cout << "\n--- Obtained Rewards ---\n";
    if (rewardCount == 0)
    { // if the number of rewards is zero, show the message
    ...
    else
    {
        ...
        { // if there are rewards, then iterate through the array and show the rewards won
...
void saveProgress()
{
    ...
    if (file.is_open())
    {
        ...
        if (adventuresPlayed[0])
            file << "Nerysia (Water World)" << endl;
        if (adventuresPlayed[1])
            file << "Infernum (Fire World)" << endl;
        if (adventuresPlayed[2])
            file << "thornia (The Forgotten World)" << endl;
        if (umbraPlayed)
            file << "Umbra (Shadow Realm)" << endl;
...
void readProgress()
{
    ...
    if (file.is_open())
    {
        ...
        for (int i = 0; i < adventures; i++)
        {
            getline(file, line);
            cout << i + 1 << ". " << line << endl;
            if (line == "Nerysia (Water World)")
                adventuresPlayed[0] = true;
            else if (line == "Infernum (Fire World)")
                adventuresPlayed[1] = true;
            else if (line == "thornia (The Forgotten World)")
                adventuresPlayed[2] = true;
            else if (line == "Umbra (Shadow Realm)")
                umbraPlayed = true;
        }
...
void minigameMagma()
{
    ...
    while (playerPoints < 2)
    {
        ...
        guardian = guardianMoves[round % 3];
...
bool playLevel(const Level &level, const MinigameConfig &config, int levelIndex)
{
    ...
    while (true)
    {
        ...
        if (input == "1" || input == "2" || input == "3")
        {
            option = stoi(input); // convert to integer
            break;
        }
        else
        {
...
void showFinalBoard()
{
    ...
    for (int i = 0; i < rows; i++)
    {
        ...
        board[i][cols] = '\0'; // last line
    }
    // message
    string message = "  YOU HAVE FINISHED THE GAME!  ";
    // Center message
    int startCol = (cols - 2 - message.length()) / 2 + 1;
    // center message
...
void gameIntroduction()
{
    ...
    do
    {
        ...
        if (onlyLetters)
            ...
            cout << "Name cannot contain numbers. Try again: ";
    } while (true);
}